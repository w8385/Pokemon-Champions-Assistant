...
export default function App() {
  ...
  }, [party, opponents, selectedMy, selectedOpp, calcAttackStage, calcDefenseStage, calcHitCount, calcWeather, calcTerrain, calcBurned, calcCritical, calcAttackerLowHp, calcTargetPoisoned, calcDefenderFullHp, calcMovedAfterTarget, calcFaintedAllies, calcRivalryMode, calcParentalBond, calcDefenderStatused, calcElectromorphosisCharged, calcReflect, calcLightScreen, calcAuroraVeil, calcFriendGuard, calcTypeChangeStab, calcConditionalPowerValues, calcOpponentBulkPreset, calcOpponentHpEv, calcOpponentDefenseEv, calcOpponentSpDefenseEv, calcOpponentDefenseNature, calcOpponentSpDefenseNature, battleNote, confirmedMovesByKey, mainSection, sampleForge, savedSamples])
  ...
  React.useEffect(() => {
    if (!oppRow?.key || typeof oppRow.id !== 'number') return
    if (typeof oppWeightKg === 'number') return
    let cancelled = false
    fetchPokemonWeightKg(oppRow.id)
      .then((weightKg) => {
        if (cancelled) return
        setWeightByKey((prev) => prev[oppRow.key] === weightKg ? prev : { ...prev, [oppRow.key]: weightKg })
      })
      .catch(() => {})
    return () => {
      cancelled = true
  ...
  const activeDamageMoveMeta = applyConditionalMovePower(
    ...
    applyTargetWeightMovePower(
      ...
      resolveAbilityAdjustedMoveMeta(
        activeDamageMove,
        resolveMultiHitMeta(activeDamageMove, activeDamageMoveBaseMeta, activeDamageMoveHitCount, selectedAttackAbility?.slug ?? myMember.ability),
        ...
        {activeTab === 'power' ? <section className="panel wide">
          ...
          <div className="damage-surface-card damage-control-surface">
            {activeDamageMoveMeta?.variablePower && !activeDamageMoveHitOptions?.length ? <div className="pick-summary-badges damage-auto-badges">
              <span className="pick-badge warn">{variablePowerHint(activeDamageMove, lt, { targetWeightKnown: typeof oppWeightKg === 'number' })}</span>
            </div> : null}
